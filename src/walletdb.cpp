...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call CBN with fAggressive=true to
    // get as much data as possible.
    // Rewrite cbnd data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> cbndData;
    bool allOK = dbenv.CBN(newFilename, true, cbndData);
    if (cbndData.empty()) {
        LogPrintf("CBN(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("CBN(aggressive) found %u records\n", cbndData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, cbndData) {
        if (fOnlyKeys) {
