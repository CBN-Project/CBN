...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CBNMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CBNMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CBNMiner started (POS=%s)\n", (fProofOfStake ? "true" : "false") );
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cbn-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (chainActive.Tip()->nTime < Params().GenesisBlock().nTime || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || fReindex || fImporting || nReserveBalance >= pwallet->GetBalance()) {
        ...
        //LogPrintf("Running CBNMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
